...
export default function SelectBingo() {
  ...
  const { mutate: setCurrentBingo, isPending: isSettingCurrentBingo } = useMutation({
    mutationKey: ['set-current-bingo', selectedBingo?.slug],
    mutationFn: async (bingo: ShortBingoDto) => {
      setSelectedBingo(bingo);
      setOpen(false);

      const response = await setCurrentBingoApi(bingo.slug);
                ...
                <CommandGroup heading={heading} key={heading}>
                  {bingos.map((item) => (
                    ...
                    >
                      ...
                      <Check className={cn('h-4 w-4', valueDisplay.slug === item.slug ? 'opacity-100' : 'opacity-0')} />
                    </CommandItem>
                  ))}
                </CommandGroup>
              ),
